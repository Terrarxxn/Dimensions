...
class TerrariaServerPacketHandler {
  ...
  private handleContinueConnecting(packet: RawPacket): boolean {
    ...
    if (!this.currentServer.isVanilla) {
      ...
      if (this.currentServer.client.useNextSpawn)
      {
        const packetData = new PacketWriter()
          .setType(PacketTypes.DimensionsUpdate)
          .packInt16(4) // Type
          .packInt16(this.currentServer.client.nextSpawnX)
          .packInt16(this.currentServer.client.nextSpawnY)
          .data;

        this.currentServer.socket.write(packetData);

        this.currentServer.client.useNextSpawn = false;
      }
    }
  ...
  private handleCompleteConnectionAndSpawn(packet: RawPacket): boolean {
    ...
    if (this.currentServer.client.state === ClientState.FinalisingSwitch) {
      ...
      let spawnX = this.currentServer.spawn.x;
      let spawnY = this.currentServer.spawn.y;

      if (this.currentServer.client.useNextSpawn)
      {
        spawnX = this.currentServer.client.nextSpawnX;
        spawnY = this.currentServer.client.nextSpawnY;
      }
  ...
  private handleDimensionsUpdate(packet: RawPacket): boolean {
    ...
    // Switch server
    if (messageType === 2) {
      const dimension: string = reader.readString();
      if (this.currentServer.client.servers[dimension.toLowerCase()]) {
        this.currentServer.client.sendChatMessage("Shifting to the " + dimension + " Dimension", "FF0000");
        this.currentServer.client.changeServer(this.currentServer.client.servers[dimension.toLowerCase()], {
    ...
    if (messageType === 3) {
      const ip: string = reader.readString();
    ...
    if (messageType === 4) {
      const spawnX: number = reader.readInt16();
      const spawnY: number = reader.readInt16();
      this.currentServer.client.useNextSpawn = true;
      this.currentServer.client.nextSpawnX = spawnX;
      this.currentServer.client.nextSpawnY = spawnY;
    }
    return true;
  }
