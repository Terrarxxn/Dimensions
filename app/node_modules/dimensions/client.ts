...
class Client {
  ...
  public useNextSpawn: boolean;
  public nextSpawnX: number;
  public nextSpawnY: number;
  ...
  public changeServer(server: RoutingServer, options?: ChangeServerOptions): void {
    ...
    this.server.afterClosed = () => {
      ...
      const stonedPacket = this.server.getPacketHandler().handlePacket(this.server, stoned);
      if (stonedPacket !== null) {
        this.send(stonedPacket);
      }
      ...
      const webbedPacket = this.server.getPacketHandler().handlePacket(this.server, webbed);
      if (webbedPacket !== null) {
        this.send(webbedPacket);
      }
